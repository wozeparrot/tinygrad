...
KERNEL_CATEGORIES = [
  ...
  # Custom fused kernels (silu/rmsnorm/pad-grad-accum/ce-loss/quantize)
  ("fused", [r"^fused_", r"^quantize_fp"]),
...
def load_profile(profile_events: list) -> dict:
  ...
  from tinygrad.viz.serve import get_profile, VizData

  ret = get_profile(VizData(), profile_events)
  if ret is None:
...
def is_gpu_kernel(name: str) -> bool:
  ...
  # GPU kernels start with: E_, r_, gemm_, fa_, embedding_bwd, fused_, quantize_fp
  # CPU events: realize, get_program, Apply, memory planner, etc.
  return bool(re.match(r"^(E_|r_|gemm_|hk_fp8_gemm_|custom_|embedding_|fused_|quantize_fp)", name))
