...
from extra.llama_kernels.embedding import GPTOSSEmbedding
from extra.gemm.cdna_asm_gemm import _mx_block_scale, _mx_block_scale_3d, quantize_mxfp8
from extra.gemm.moe_gemm import grouped_mx_gemm, grouped_mx_gemm_swiglu, mx_pack_3d, FUSED_FC1_COLW, PRESTORE_WT
from extra.gemm.moe_routing import route, dispatch, dispatch_fp8, combine, router_mfma, router_quantize
...
def matmul_mx(x:Tensor|tuple[Tensor, Tensor], w_q:Tensor, w_scale:Tensor) -> Tensor:
  ...
  if ASM_GEMM:
    ...
    if (npad := (-N) % 256):
      wq = wq.pad(((0, npad), (0, 0)))
      ws = ws.pad(((0, npad), (0, 0)), value=127).cast(dtypes.uint8)
    # Exact GPT-OSS attention-output projection input.  Produce q/e8 together so the 1x32 amax source is read once;
    # keep mx_pack separate because its small transposed scale write is faster than the high-VGPR all-in-one producer.
    local_rows = x2.shape[0] // len(x2.device) if isinstance(x2.device, tuple) and x2.uop.axis == 0 else x2.shape[0]
    if getenv("GPTOSS_ATTN_FUSED_QE8", 0) and (local_rows, x2.shape[1]) == (16384, 4096) \
       and w_q.shape == (2880, 4096) and x2.dtype == dtypes.bfloat16:
...
def _moe_bias_tile(bias:Tensor, r) -> Tensor:
  # per-grouped-row expert bias done at TILE granularity: all 256 rows of a grouped tile share one expert
  # (rows_e = tile_e expanded x256), so the onehot@bias runs over ~m_l/256 TILES not m_l rows -- 256x less
  # work than the per-row `onehot @ bias` (whose backward was the 150ms r_18_2_90; the per-row GATHER instead
  # blew up to 404ms of atomic-scatter contention). fwd = tiny (tiles,E)@(E,D) matmul + broadcast that FUSES
  # into the +bias add (no 850MB materialize); bwd = the expand's sum(256) (contention-free reduce) + the tiny
  # tile-onehot^T contraction, which ALLREDUCES the replicated bias grad via the sharded-tiles axis (like the
  # baseline onehot). Returns fp32 (E,D)-broadcast rows; caller casts to bf16.
  D = bias.shape[1]
  tile_bias = r.tile_e.one_hot(bias.shape[0]).float() @ bias.float()   # (G*tiles, D)
  return tile_bias.reshape(-1, 1, D).expand(-1, 256, D).reshape(-1, D)  # broadcast tiles -> (G*m_l, D)
...
class GPTOSS:
  def __init__(self, dim:int, n_layers:int, n_heads:int, n_kv_heads:int, head_dim:int, n_experts:int, experts_per_tok:int,
    ...
    if PRESTORE_WT:   # pre-store transposed mxfp8 weights so the dgrad skips dequant+transpose+requant (companion state, not params)
  ...
  def _make_wT(self, ws:list, scs:list):
    # per-layer pre-stored transposed mxfp8 weight (W^T=(E,K,N) fp8 + e8 along N). Init parity: quantize the transpose of
    # the DEQUANTIZED stored w_q (NOT the pre-quant weight), so the step-0/1 dgrads (which read the init weight) match the
    # runtime dequant+transpose+requant byte-for-byte. Companion state (is_param_(False)+_prestore_wT), replicated like w.
    wtq, wte = [], []
    for w_q, w_e8 in zip(ws, scs):
  ...
  def attention(self, x:Tensor, freqs_cis:Tensor, mask:Tensor, sliding:bool, *, attention_norm:Tensor, wqkv:Tensor,
    ...
    if getenv("FUSED_RMSNORM_MX", 0):
      from extra.llama_kernels.fused_rmsnorm_mul_quantize_mxfp8 import rmsnorm_mul_quantize_mxfp8
  ...
  def feed_forward(self, x:Tensor, *, ffn_norm:Tensor, gate:Tensor, gate_bias:Tensor,
                   ...
                   router_topk_out:tuple[Tensor, Tensor]|None=None):
    ...
    if grouped_moe:
      ...
      return out, [x_normed, rrms, *xg_save, *h_save, y, z, r.weights, r.topi, r.dest_row, r.off, r.counts.contiguous()]
    else:
  ...
  def run_layer(self, x:Tensor, freqs_cis:Tensor, mask:Tensor, sliding:bool, attn_kwargs:dict, ffn_kwargs:dict, save:bool=True):
    ...
    if getenv("DIRECT_ROUTER_TOPK_OUT", 0) and getenv("FUSED_ROUTER_TOPK", 0) and getenv("GROUPED_MOE", 0):
      ...
      ffn_kwargs["router_topk_out"] = (
        _sharded_invalids((bsz * seqlen, self.experts_per_tok), dtypes.float32, h.device).clone(),
        _sharded_invalids((bsz * seqlen, self.experts_per_tok), dtypes.int32, h.device).clone())
    ...
    if save and getenv("GPTOSS_SAVE_FFN_INPUT", 0): ffn_saves.append(h)
  ...
  def __call__(self, tokens:Tensor, save:bool=True, targets:Tensor|None=None):
    ...
    if getenv("GPTOSS_FINAL_RMSNORM", 0):
      from extra.llama_kernels.rmsnorm import gptoss_final_rmsnorm
