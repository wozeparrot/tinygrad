...
# Use upstream HCQ2 scheduling; no HCQ1 reorder hooks or measured cost tables.
export HCQ2=${HCQ2:-1}
export CHECK_OOB=0
...
export GPTOSS_FC1_WGRAD_NATIVE32=${GPTOSS_FC1_WGRAD_NATIVE32:-1}
...
# Exact GPT-OSS attention residual + FFN RMS producer, retaining native backward checkpoints.
export GPTOSS_RESIDUAL_RMSNORM_FWD=${GPTOSS_RESIDUAL_RMSNORM_FWD:-1}
...
export RMSNORM_MX_EP8=${RMSNORM_MX_EP8:-1}
...
export FUSED_ATTN_QE8=${FUSED_ATTN_QE8:-1}
...
export FAST_FINAL_RMSNORM=${FAST_FINAL_RMSNORM:-1}
...
# Native per-destination expert gradients avoid 336 reduce-scatter staging kernels/GPU/update on DP8.
export GPTOSS_WGRAD_SHARDS=${GPTOSS_WGRAD_SHARDS:-1}
...
# Benchmark-only defaults; keep the training flags above aligned with dev_run.sh.
