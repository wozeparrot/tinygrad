...
def train_gptoss():
  ...
  # PRESTORE_WT: attach the pre-stored W^T companions (name-resolved to the canonical sharded model_state object, NEVER a
  # fresh tensor -> double-shard guard) so the optimizer writes them (hasattr(t,'_wT_q')) and they realize each step.
  fp8_wT_tensors = []
  if getenv("PRESTORE_WT", 0):
    def _wt_key(n, suffix):  # mirror _scale_key: handle both `X.{i}` (-> X_{suffix}.{i}) and dotless `X` (-> X_{suffix})
  ...
  from examples.mlperf.gptoss_training import DeferredLMHead, next_group_size
  ...
  try:
    while i < MAX_STEPS:
      ...
      if getenv("TRAIN", 1):
        ...
        mem_gb = GlobalCounters.mem_used / 1e9
        gflops = GlobalCounters.global_ops / 1e9 / dev_time
        mfu = ((6 * num_params * SEQLEN * GBS * count) / (dev_time * device_count * 4.6e15)) * 100
