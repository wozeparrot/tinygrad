#if !defined(ATTN_D) || ATTN_D != 64
#error "fa_bwd_causal_d64.cpp only supports ATTN_D=64"
#endif

#if WINDOW
#error "fa_bwd_causal_d64.cpp is the GPT-OSS full-causal specialization"
#endif
...
#ifndef GPTOSS_FA_BWD_RELAX_DQ_SYNC
#define GPTOSS_FA_BWD_RELAX_DQ_SYNC 0
#endif
#if GPTOSS_FA_BWD_RELAX_DQ_SYNC
#if ATTN_B != 2 || ATTN_N != 8192 || ATTN_H != 64 || ATTN_H_KV != 8 || ATTN_D != 64 || WINDOW != 0 || BWD_BLOCK_SIZE_KV != 256 || NUM_WARPS != 4 || BWD_HEADS_PER_WG != 2
#error "Relaxed dQ synchronization is qualified only for the exact full-causal GPTOSS backward shape"
#endif
// The retained publication/reuse synchronization protects shared dS.
// This intermediate point only separates wave-local register work; retain all waits.
#define DQ_MID_SYNC() ((void)0)
#else
#define DQ_MID_SYNC() __builtin_amdgcn_s_barrier()
#endif

#ifndef GPTOSS_FA_BWD_DOUBLE_ATTN
#define GPTOSS_FA_BWD_DOUBLE_ATTN 0
#endif
#if GPTOSS_FA_BWD_DOUBLE_ATTN
#if !GPTOSS_FA_BWD_RELAX_DQ_SYNC
#error "Double dS scratch requires the exact qualified GPTOSS full-causal schedule"
#endif
// Alternate dS planes. The next slice's publication fence releases this plane
// before its next writer two slices later; all memory waits remain.
#define DQ_REUSE_SYNC() ((void)0)
#define DQ_ODD_PLANE_OFFSET 8192u
#else
#define DQ_REUSE_SYNC() __builtin_amdgcn_s_barrier()
#define DQ_ODD_PLANE_OFFSET 0u
#endif
...
template<int I, int J, typename RT, typename ST>
__device__ __forceinline__ void reload_k(const RT &dst, const ST &src, uint32_t addr) {
  if constexpr (WINDOW) load<I, J>(dst, src, addr);
}

template<int GPR_START>
__device__ __forceinline__ void global_store_dwordx2_sbase(bf16 *base, uint32_t byte_offset) {
  static_assert(GPR_START >= 0 && GPR_START + 1 < 256);
  asm volatile("global_store_dwordx2 %0, v[%1:%2], %3"
               : : "v"(byte_offset), "n"(GPR_START), "n"(GPR_START + 1), "s"(base) : "memory");
}

// Store one wave's fixed-register 64x64 accumulator tile directly to the
// GPT-OSS partial dK/dV layout. The generic tile store rebuilds these same
// coordinates while carrying a full buffer resource through the kernel.
template<ducks::art::all RT>
__device__ __forceinline__ void store_d64_warp64_global(bf16 *dst_ptr, const RT &src,
                                                        int partial_idx, int seq_idx,
                                                        int kv_head_idx, int uniform_warpid) {
  static_assert(std::is_same_v<typename RT::layout, ducks::rt_layout::row> && std::is_same_v<typename RT::shape, rt_32x32_s>);
  static_assert(std::is_same_v<typename RT::T, float> && RT::height == 2 && RT::width == 2);
  (void)src;

  const int lane_row = kittens::laneid() % RT::base_tile_rows;
  const int lane_col = RT::base_tile_stride * (kittens::laneid() / RT::base_tile_rows);
  bf16 *wave_ptr = dst_ptr + (((partial_idx * ATTN_N + seq_idx * BLOCK_SIZE_KV +
                               uniform_warpid * WARP_SIZE_KV) * ATTN_H_KV + kv_head_idx) * ATTN_D);

  auto store_tile = [&]<int N, int M>() {
    using tile_range = ducks::art::get_nth_range_t<typename RT::register_ranges, N * RT::width + M>;
    constexpr int R0 = tile_range::lo;
    constexpr int R1 = tile_range::lo + 4;
    constexpr int R2 = tile_range::lo + 8;
    constexpr int R3 = tile_range::lo + 12;
    constexpr int B0 = tile_range::lo;
    constexpr int B1 = tile_range::lo + 2;
    constexpr int B2 = tile_range::lo + 4;
    constexpr int B3 = tile_range::lo + 6;

    macros::v_cvt_pk_bf16_f32<B0, R0, R0 + 1>();
    macros::v_cvt_pk_bf16_f32<B0 + 1, R0 + 2, R0 + 3>();
    macros::v_cvt_pk_bf16_f32<B1, R1, R1 + 1>();
    macros::v_cvt_pk_bf16_f32<B1 + 1, R1 + 2, R1 + 3>();
    macros::v_cvt_pk_bf16_f32<B2, R2, R2 + 1>();
    macros::v_cvt_pk_bf16_f32<B2 + 1, R2 + 2, R2 + 3>();
    macros::v_cvt_pk_bf16_f32<B3, R3, R3 + 1>();
    macros::v_cvt_pk_bf16_f32<B3 + 1, R3 + 2, R3 + 3>();

    auto store_chunk = [&]<int COL_OFFSET, int GPR>() {
      const int row = N * RT::base_tile_rows + lane_row;
      const int col = M * RT::base_tile_cols + lane_col + COL_OFFSET;
      const uint32_t byte_offset = (row * ATTN_H_KV * ATTN_D + col) * sizeof(bf16);
      global_store_dwordx2_sbase<GPR>(wave_ptr, byte_offset);
    };
    store_chunk.template operator()<0, B0>();
    store_chunk.template operator()<8, B1>();
    store_chunk.template operator()<16, B2>();
    store_chunk.template operator()<24, B3>();
  };
  store_tile.template operator()<0, 0>();
  store_tile.template operator()<0, 1>();
  store_tile.template operator()<1, 0>();
  store_tile.template operator()<1, 1>();
}
...
__global__ __attribute__((amdgpu_num_vgpr(29))) void attend_bwd_combined_ker(bf16 *dQ_ptr, bf16 *dK_ptr, bf16 *dV_ptr, bf16 *dO_ptr, bf16 *Q_ptr, bf16 *K_ptr, bf16 *V_ptr, float *L_vec_ptr, float *delta_vec_ptr) {
  ...
  // The warp id is lane-invariant, but deriving it from threadIdx makes LLVM
  // conservatively keep k_pos in a VGPR. Mark the value uniform so the causal
  // block tests use scalar control flow instead of saveexec/restoreexec paths.
  const int uniform_warpid = __builtin_amdgcn_readfirstlane(warpid);
  const int j = seq_idx * NUM_WARPS + uniform_warpid;
...
#if WINDOW
  // GPT-OSS WINDOW=128 needs exactly six Q steps per 256-key block. The old shared D=64/128
  // schedule padded this with two fully masked steps to flush dQ; the dedicated D=64 epilogue does not need them.
  ...
  constexpr float P_SCALE_FACTOR = 0.125f*1.44269504089f;
  constexpr float dP_SCALE_FACTOR = 0.125f;
  ...
  #if GPTOSS_FA_BWD_DOUBLE_ATTN
  st_bf<BLOCK_SIZE_KV, DOT_SLICE_QO, st_16x16_swizzled_s> (&attn_i_smem_storage)[2] = al.allocate<st_bf<BLOCK_SIZE_KV, DOT_SLICE_QO, st_16x16_swizzled_s>, 2>();
  auto &attn_i_smem = attn_i_smem_storage[0];
  static_assert(sizeof(attn_i_smem) == 8192);
#else
  ...
  // Register tiles
  using Q_ranges = ducks::art::split_many_t<ducks::art::type_list<ducks::art::range<102, 109>>, 4>;
  ...
  using K_ranges = ducks::art::split_many_t<ducks::art::type_list<ducks::art::range<256, 263>, ducks::art::range<264, 271>, ducks::art::range<272, 279>, ducks::art::range<280, 287>>, 4>;
  using V_ranges = ducks::art::split_many_t<ducks::art::type_list<ducks::art::range<288, 295>, ducks::art::range<296, 303>, ducks::art::range<304, 311>, ducks::art::range<312, 319>>, 4>;
  ...
  using dK_ranges = ducks::art::split_many_t<ducks::art::type_list<ducks::art::range<320, 383>>, 16>;
  ...
  art<bf16, BLOCK_SIZE_KV, ATTN_D/4, col_l, rt_32x16_4_s, Kcol_ranges> K_j_col;
  art<bf16, BLOCK_SIZE_KV, 16, col_l, rt_32x16_4_s, dS_ranges> dP_ij_bf16_col_T; // 32 registers // for dq
  ...
  attn_bwd_combined_globals<D> g{Q, K, V, dOg, dQg, L_vec_gl, delta_vec_gl};
  ...
  // uint32_t K_j_col_addr = get_address(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}));
  const uint32_t K_j_col_addr = [&] {
    const int laneid = kittens::laneid();
    const uint32_t src_ptr = reinterpret_cast<uintptr_t>(&subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}).data[0]);
  ...
  // Invariant dQ K operand; fixed v192:v223 are not aliased by another tile.
  load<0, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<1, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<2, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<3, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<4, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<5, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<6, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  load<7, 0>(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}), K_j_col_addr);
  ...
  if (num_steps > 1) {
    // Prologue
    {
...
#if WINDOW
      const bool kv_active = static_cast<uint32_t>(q_pos - k_pos) <= WINDOW;
#else
      // Later waves in this 256-key block have one to three fully causal-masked
      // query tiles at the triangular edge. Skip their score/dP work while all
      // waves continue through the block-wide barriers and dQ reduction.
      const bool kv_active = q_pos >= k_pos;
#endif
      ...
      // dot slice 0
      {
        ...
        kittens::macros::v_mov_b32_up2p<neg_inf_v>(0xff800000);
        if constexpr (causal) {
          // Only the diagonal key tile needs masking; fully future tiles were rejected by kv_active.
          if (q_pos == k_pos) {
        ...
        swap_layout_inplace(P_ij_bf16_col, P_ij_bf16);
          ...
          // Full-causal keeps K resident across query steps. The normal active
          // prologue fills its upper half while computing scores; an inactive
          // triangular-edge wave must still initialize those registers for the
          // first later query tile that becomes active.
          if constexpr (!WINDOW) {
            load<2, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
            load<2, 1>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
            load<3, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        ...
        // load(K_j_col, subtile_inplace<BLOCK_SIZE_KV, ATTN_D/4>(K_j_smem, {0, warpid}));
        // K_j_col is invariant and resident in v192:v223.
        ...
        asm volatile("s_waitcnt lgkmcnt(6)"); // resident-Kcol: wait for Q_i_col
        ...
        // Load K_j from shared memory to registers
        // load(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}));
        reload_k<0, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        reload_k<0, 1>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        reload_k<1, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        reload_k<1, 1>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
      ...
      // dot slice 1
      {
        ...
        if (kv_active) {
        mma_ABt<0, 0, 0>(P_ij, Q_i, K_j);
        reload_k<2, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        reload_k<2, 1>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        mma_ABt<0, 0, 1>(P_ij, Q_i, K_j, P_ij);
        mma_ABt<0, 1, 0>(P_ij, Q_i, K_j);
        reload_k<3, 0>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        reload_k<3, 1>(K_j, subtile_inplace<WARP_SIZE_KV, D>(K_j_smem, {warpid, 0}), K_j_addr);
        mma_ABt<0, 1, 1>(P_ij, Q_i, K_j, P_ij);
        mul<0, 0>(P_ij, P_ij, P_SCALE_FACTOR);
        asm volatile("s_waitcnt lgkmcnt(6)");
        mma_ABt<0, 2, 0>(P_ij, Q_i, K_j);
        ...
        // Store dP_ij_bf16_accum_row to shared memory
        // store(attn_i_smem_subtile, dP_ij_bf16_accum_row);
        store<0, 0>(attn_i_smem_subtile, dP_ij_bf16_accum_row, (dP_ij_bf16_accum_row_addr + DQ_ODD_PLANE_OFFSET));
        store<1, 0>(attn_i_smem_subtile, dP_ij_bf16_accum_row, (dP_ij_bf16_accum_row_addr + DQ_ODD_PLANE_OFFSET));
        sub_row<0, 3, delta_i>(dP_ij, dP_ij);
        mul<0, 2>(dP_ij, dP_ij, P_ij);
        mul<0, 3>(dP_ij, dP_ij, P_ij);
        copy<0, 2>(dP_ij_bf16, dP_ij);
        ...
        store<2, 0>(attn_i_smem_subtile, dP_ij_bf16_accum_row, (dP_ij_bf16_accum_row_addr + DQ_ODD_PLANE_OFFSET));
        store<3, 0>(attn_i_smem_subtile, dP_ij_bf16_accum_row, (dP_ij_bf16_accum_row_addr + DQ_ODD_PLANE_OFFSET));
        swap_layout_inplace(dP_ij_bf16_col, dP_ij_bf16);
        asm volatile("s_waitcnt lgkmcnt(6)"); // resident-Kcol: wait for Q_i_col
        ...
        // Load dP_ij_bf16_col_T from shared memory to registers
        // load(dP_ij_bf16_col_T, attn_i_smem);
        load<0, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        load<1, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        load<2, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        load<3, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        mma_AtB<1, 1, 0>(dK_j_T, Q_i_col, dP_ij_bf16_col, dK_j_T);
        mul<L_i, L_i>(L_SCALE_FACTOR);
        // Scale at the deferred atomic consumer, after the MFMA result has naturally cleared its
        // 16-cycle accumulator-to-VALU hazard. Scaling at the producer required an explicit s_nop 15.
        ...
        load<4, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        load<5, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        // K_j_col is invariant and resident in v192:v223.
        load<6, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
        load<7, 0>(dP_ij_bf16_col_T, attn_i_smem, (dP_ij_bf16_col_T_addr + DQ_ODD_PLANE_OFFSET));
    ...
    int last_q_head_idx = first_q_head;
    int last_q_seq_idx = first_step;
    int q_head_idx = first_q_head;
    int q_seq_idx = first_step + 1;
    int q_pos = q_seq_idx * STEP_QO;
    int next_q_head_idx = first_q_head;
    int next_q_seq_idx = first_step + 2;
    const int q_seq_end = first_step + num_steps_per_head;
    if (next_q_seq_idx == q_seq_end) {
      next_q_seq_idx = first_step;
      ++next_q_head_idx;
    }

    // 9. for 1 <= i <= T_r (1024 / 32 = 32)  
    for (int i = 1; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
      auto iteration = [&]<bool HOT>() {
#if WINDOW
      const bool kv_active = static_cast<uint32_t>(q_pos - k_pos) <= WINDOW;
#else
      const bool kv_active = HOT || q_pos >= k_pos;
#endif
      ...
      last_q_head_idx = q_head_idx;
      last_q_seq_idx = q_seq_idx;
      };
      if (q_pos >= k_start_min + BLOCK_SIZE_KV) iteration.template operator()<true>();
      else iteration.template operator()<false>();
      q_head_idx = next_q_head_idx;
      q_seq_idx = next_q_seq_idx;
      q_pos = q_seq_idx * STEP_QO;
      if (++next_q_seq_idx == q_seq_end) {
        next_q_seq_idx = first_step;
        ++next_q_head_idx;
      }
  ...
  const bool kv_active = static_cast<uint32_t>(q_pos - k_pos) <= WINDOW;
#else
  // The epilogue handles the final query tile of the final head, which is
  // always at or beyond every key wave in this workgroup.
  constexpr bool kv_active = true;
#endif
  // Epilogue
  {
    // dot slice 0
    {
      ...
      // K_j_col is invariant and resident in v192:v223.
      mma_AtB<0, 1, 0>(dK_j_T, Q_i_col, dP_ij_bf16_col, dK_j_T);
      // The last main iteration has no following VM wait to finish its dO half1 prefetch.
      // Complete it before this existing publication barrier, ahead of epilogue slice2.
    ...
    // dot slice 3
    {
      ...
      store<3, 0>(attn_i_smem_subtile, dP_ij_bf16_accum_row, (dP_ij_bf16_accum_row_addr + DQ_ODD_PLANE_OFFSET));
      swap_layout_inplace(dP_ij_bf16_col, dP_ij_bf16);
      asm volatile("s_waitcnt lgkmcnt(4)"); // resident-Kcol: wait for Q_i_col
      ...
      mma_AtB<0, 0, 5>(dQ_i_T, K_j_col, dP_ij_bf16_col_T, dQ_i_T);
      asm volatile("s_waitcnt lgkmcnt(0)"); // final dQ: no next-Q LDS suffix
      __builtin_amdgcn_s_barrier();
      mma_AtB<0, 0, 6>(dQ_i_T, K_j_col, dP_ij_bf16_col_T, dQ_i_T);
  ...
  const int partial_idx = batch_idx * (GROUP_SIZE / HEADS_PER_WG) + q_head_in_group / HEADS_PER_WG;
  store_d64_warp64_global(dV_ptr, dV_j, partial_idx, seq_idx, kv_head_idx, uniform_warpid);
  __builtin_amdgcn_s_waitcnt(0);
  __builtin_amdgcn_s_barrier();
  ...
  store_d64_warp64_global(dK_ptr, dV_j, partial_idx, seq_idx, kv_head_idx, uniform_warpid);
