...
def flash_attention(xq, xk, xv, attn_mask:Tensor|None=None, is_causal:bool=False):
  ...
  def grad(dou:UOp, _) -> tuple[None, None, UOp, UOp, UOp]:
    ...
    dk = _sharded_empty((B, N, H_KV, D), xk, axis=shard_axis)
    dv = _sharded_empty((B, N, H_KV, D), xv, axis=shard_axis)

    # Zero dk/dv via a tracked kernel (needed for atomic accumulation; must be in the graph for JIT correctness)
    def _zero_kernel(out:UOp) -> UOp:
      i = UOp.range(out.size, 0)
      return out.flatten()[i].store(0).end(i).sink(arg=KernelInfo(name="zero"))
    dk = Tensor(dk.uop.custom_kernel(fxn=_zero_kernel)[0], device=dk.device)
    dv = Tensor(dv.uop.custom_kernel(fxn=_zero_kernel)[0], device=dv.device)
    ...
    dq, dk, dv = Tensor.custom_kernel(dq, dk, dv, do, xq, xk, xv, l_vec, delta_vec, fxn=functools.partial(custom_fa_backward, device=single_device, arch=arch, B=B_local, N=N, H=H_local, H_KV=H_KV_local, D=D))[:3]
