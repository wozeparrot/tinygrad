...
__global__ __launch_bounds__(512, 2) void grouped_mxfp8_wgrad_kernel(bf16 *C_ptr, fp8e4m3 *A_ptr, fp8e4m3 *B_ptr,
    ...
    #define COMPUTE_WGRAD_STAGE(P)                                                                                       \
        fp8e8m0_4 sa_h0 = pack_scales(scale_A_smem[P].data, a_row_h0);                                                   \
        fp8e8m0_4 sa_h1 = pack_scales(scale_A_smem[P].data, a_row_h1);                                                   \
        fp8e8m0_4 sb_h0 = PACK_B_SCALES(scale_B_smem[P].data, b_row_h0);                                                 \
        fp8e8m0_4 sb_h1 = PACK_B_SCALES(scale_B_smem[P].data, b_row_h1);                                                 \
        ISSUE_WGRAD_OPERANDS(P)                                                                                         \
        asm volatile("s_waitcnt lgkmcnt(0)");                                                                            \
        if (live_n_warp && live_k_warp) mma_ABt_scaled(cA, a, b0, cA, &sa_h0, &sb_h0);                                  \
        if (live_n_warp && !quarter_k_tail) mma_ABt_scaled(cB, a, b1, cB, &sa_h0, &sb_h1);                              \
        if (!half_n_tail) {                                                                                              \
            auto as1 = subtile_inplace<REG_M, BLOCK_K>(As[P][1], {warp_m, 0}); load(a, as1);                             \
            asm volatile("s_waitcnt lgkmcnt(0)");                                                                        \
    ...
    #define LOAD_WGRAD_STAGE1_FULL(KK)                                                                                   \
        ...
        G::load(scale_B_smem[1], scale_B_gl, {(KK) * tiles_K + block_col, 0, 0, 0});
#endif
    #define COMPUTE_WGRAD_STAGE_FULL(P)                                                                                  \
        fp8e8m0_4 sa_h0 = pack_scales(scale_A_smem[P].data, a_row_h0);                                                  \
        fp8e8m0_4 sa_h1 = pack_scales(scale_A_smem[P].data, a_row_h1);                                                  \
        fp8e8m0_4 sb_h0 = PACK_B_SCALES(scale_B_smem[P].data, b_row_h0);                                                \
        fp8e8m0_4 sb_h1 = PACK_B_SCALES(scale_B_smem[P].data, b_row_h1);                                                \
        ISSUE_WGRAD_OPERANDS_FULL(P)                                                                                    \
        COMPUTE_WGRAD_H0()                                                                                              \
