...
// The reference reduction assigns four independent rows to each lane. One row per lane exposes four times as many
// waves while retaining each row's exact left-to-right groups-of-four accumulation and its sqrt denominator ABI.
static __device__ __forceinline__ float exact_saved_rrms(float x) {
  // Fast reciprocal plus one Newton step. The explicit non-finite path is retained by -fno-finite-math-only.
  // RMSNorm's eps keeps every finite production denominator normal, so no subnormal-reciprocal path is needed.
