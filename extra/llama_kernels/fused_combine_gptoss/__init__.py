...
from extra.llama_kernels import FP8_MAX, THREADS_PER_WG, alloc_like, compile_hip, dname_of
...
def _combine_bwd(gradient:UOp, kernel:UOp):
  ...
  if os.getenv("GPTOSS_COMBINE_DZ_QUANT", "0") == "1":
    ...
    outputs = [alloc_like(shape, dtype, dev, axis).clone() for shape, dtype, axis in (
