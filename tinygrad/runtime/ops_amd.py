...
from tinygrad.helpers import getenv, to_mv, round_up, data64_le, all_same, flatten, DEBUG, PROFILE, ProfileEvent
...
class AMDDevice(HCQCompiled):
  ...
  def __init__(self, device:str=""):
    ...
    super().__init__(device, AMDAllocator(self), AMDLLVMRenderer(self.arch) if getenv("LLVM", 1) else AMDRenderer(self.arch),
                     AMDLLVMCompiler(self.arch) if getenv("LLVM", 1) else HIPCompiler(self.arch), functools.partial(AMDProgram, self),
