...
class Linearizer(OptimizedKernel):
  ...
  def linearize(self):
    ...
    if self.opts.has_local:
      self.global_size, self.local_size = [x.max+1 for x in loop_global_idxs][::-1], [x.max+1 for x in (warp_idxs+loop_local_idxs)][::-1]
      ...
      self.loop_uops.update({x.expr:self.uop(UOps.SPECIAL, dtypes.int32, (), (len(warp_idxs+loop_local_idxs)-1-i, x.expr, x.max+1)) for i,x in enumerate(warp_idxs+loop_local_idxs)})
    else:
      ...
      if self.tensor_core:
        ...
        for i in range(len(acc)//wmma_sz[2]):
          for j in range(acc_reds):
            self.uop(UOps.WMMA, None, tuple(wmma_vals[0][(i*acc_reds+j)*wmma_sz[0]:(i*acc_reds+j+1)*wmma_sz[0]] + wmma_vals[1][(i*acc_reds+j)*wmma_sz[1]:(i*acc_reds+j+1)*wmma_sz[1]] + acc[i*wmma_sz[2]:(i+1)*wmma_sz[2]]), [self.bufs[0].device, self.tensor_core.dtype_in, self.tensor_core.dtype_out])
      else:
        # load earlybufs
