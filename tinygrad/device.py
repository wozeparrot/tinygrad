...
class _Device:
  ...
  def __get_canonicalized_item(self, ix:str) -> Compiled:
    assert ALLOW_DEVICE_USAGE or ix.split(":")[0] in ["DISK", "NPY", "HTTP", "PYTHON"], f"usage of device {ix} disallowed"
  ...
  def DEFAULT(self) -> str:
    ...
    from_env = dedup(dev + [d for d in self._devices if d not in ["DISK", "NPY", "HTTP"] and getenv(d) == 1])
